...
func createRandomTransfer(t *testing.T, account1 Account, account2 Account) Transfer {
	...
	tranfer1, err := testQueries.CreateTransfer(context.Background(), arg)

	require.NoError(t, err)
	require.NotEmpty(t, tranfer1)

	require.Equal(t, tranfer1.FromAccountID, account1.ID)
	require.Equal(t, tranfer1.ToAccountID, account2.ID)
	require.Equal(t, tranfer1.Amount, arg.Amount)
	require.NotZero(t, tranfer1.CreateAt)
